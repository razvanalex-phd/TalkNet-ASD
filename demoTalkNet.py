...
from TalkNetASD.model.faceDetector.s3fd import S3FD
...
parser.add_argument(
    "--videoName", type=str, default="001", help="Demo video name"
)
parser.add_argument(
    "--videoFolder",
    type=str,
...
if args.evalCol == True:
    ...
    if (
        os.path.isdir(args.videoFolder + "/col_labels") == False
    ):  # Download label
    ...
    args.videoPath = glob.glob(
        os.path.join(args.videoFolder, args.videoName + ".*")
    )[0]
    args.savePath = os.path.join(args.videoFolder, args.videoName)
...
def scene_detect(args):
    video = open_video(args.videoFilePath, backend="pyav")
    ...
    if scene_list == []:
        scene_list = [
            (
                video.base_timecode,
                video.base_timecode + video.duration.get_seconds(),
            )
        ]
    ...
    with open(savePath, "wb") as fil:
        ...
        sys.stderr.write(
            "%s - scenes detected %d\n" % (args.videoFilePath, len(scene_list))
        )
    return scene_list
...
def inference_video(args):
    ...
    DET = S3FD("model/model.pth", device="cuda")
    ...
    for fidx, fname in enumerate(flist):
        ...
        bboxes = DET.detect_faces(
            torch.tensor([imageNumpy], device="cuda"), conf_th=0.9, scales=[args.facedetScale]
        )
        dets.append([])
...
def crop_video(args, track, cropFile):
    ...
    flist = glob.glob(
        os.path.join(args.pyframesPath, "*.jpg")
    )  # Read the frames
    flist.sort()
    with iio.imopen(cropFile + "t.mp4", "w", plugin="pyav") as vOut:
        vOut.init_video_stream("libx264", fps=25)
        ...
        for fidx, frame in enumerate(track["frame"]):
            ...
            vOut.write(
                [cv2.resize(cv2.cvtColor(face, cv2.COLOR_BGR2RGB), (224, 224))]
            )
    ...
    output = subprocess.call(
        command, shell=True, stdout=None
    )  # Crop audio file
    _, audio = wavfile.read(audioTmp)
    command = (
        "ffmpeg  -i %st.mp4 -i %s -threads %d -c:v copy -c:a aac %s.mp4"
        " -loglevel panic"
...
def evaluate_network(files, args):
    ...
    for file in tqdm.tqdm(files, total=len(files)):
        fileName = os.path.splitext(file.split("/")[-1])[
            0
        ]  # Load audio and video
        _, audio = wavfile.read(
            os.path.join(args.pycropPath, fileName + ".wav")
        )
        ...
        videoFeature = []
        with iio.imopen(
            os.path.join(args.pycropPath, fileName + ".mp4"), "r", plugin="pyav"
        ) as video:
            frames = video.read()
            for frame in frames:
        ...
        allScore = numpy.round(
            (numpy.mean(numpy.array(allScore), axis=0)), 1
        ).astype(float)
...
def visualization(tracks, scores, args):
    ...
    with iio.imopen(
        os.path.join(args.pyaviPath, "video_only.mp4"), "w", plugin="pyav"
    ) as vOut:
        vOut.init_video_stream("libx264", fps=25)
        colorDict = {0: 0, 1: 255}
        for fidx, fname in tqdm.tqdm(enumerate(flist), total=len(flist)):
            ...
            vOut.write([cv2.cvtColor(image, cv2.COLOR_BGR2RGB)])

    command = (
        "ffmpeg -y -i %s -i %s -threads %d -c:v copy -c:a aac %s -loglevel"
        " panic"
        % (
            os.path.join(args.pyaviPath, "video_only.mp4"),
            os.path.join(args.pyaviPath, "audio.wav"),
            args.nDataLoaderThread,
            os.path.join(args.pyaviPath, "video_out.mp4"),
        )
    )
    output = subprocess.call(command, shell=True, stdout=None)
...
def main():
    ...
    # │   ├── video_only.mp4 (Output video without audio)
    # │   └── video_out.mp4  (Output video with audio)
    ...
    if args.duration == 0:
        command = (
            "ffmpeg -y -i %s -qscale:v 2 -threads %d -async 1 -r 25 %s"
            " -loglevel panic"
            % (args.videoPath, args.nDataLoaderThread, args.videoFilePath)
        )
    else:
        command = (
            "ffmpeg -y -i %s -qscale:v 2 -threads %d -ss %.3f -to %.3f -async 1"
            " -r 25 %s -loglevel panic"
            % (
                args.videoPath,
                args.nDataLoaderThread,
                args.start,
    ...
    command = (
        "ffmpeg -y -i %s -qscale:a 0 -ac 1 -vn -threads %d -ar 16000 %s"
        " -loglevel panic"
